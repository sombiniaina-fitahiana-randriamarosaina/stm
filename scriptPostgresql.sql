...
CREATE OR REPLACE VIEW STATOFFREJOURNALIER AS
    SELECT CAST(JOUR AS VARCHAR) AS LIBELE, CAST(COALESCE(NOMBRE, 0) AS INTEGER) AS NOMBRE FROM JOUR NATURAL LEFT JOIN (SELECT EXTRACT(DOW FROM DATEACHAT) JOUR, COUNT(*) NOMBRE FROM ACHATOFFRE GROUP BY EXTRACT(DOW FROM DATEACHAT)) R;
...
DROP TABLE APPEL;
DROP SEQUENCE APPEL_SEQ;

DROP TABLE OFFRE;
DROP SEQUENCE OFFRE_SEQ;
...
FROM 
	JOUR NATURAL LEFT JOIN 
	(SELECT IDOFFRE, EXTRACT(DOW FROM DATEACHAT) JOUR, COUNT(*) NOMBRE FROM ACHATOFFRE GROUP BY EXTRACT(DOW FROM DATEACHAT), IDOFFRE);
