...
CREATE OR REPLACE FUNCTION CLIENT_TRIGGER() RETURNS TRIGGER AS $TRG$
    BEGIN
        INSERT INTO DATA (IDCLIENT, IDOFFRE, IDFORFAIT, DATA) SELECT NEW.IDCLIENT, IDOFFRE, IDFORFAIT, 0 FROM OFFREFORFAIT WHERE IDOFFRE = 'DEFAUT';
        INSERT INTO MOBILEMONEY (IDCLIENT, DATA) VALUES (NEW.IDCLIENT, 0);
        RETURN NEW;
    END;
...
SELECT * FROM 
    CLIENT NATURAL JOIN 
    (SELECT IDCLIENT, MAX(DATA) AS CREDIT FROM DATA WHERE IDOFFRE='DEFAUT' GROUP BY IDCLIENT ) CREDIT NATURAL JOIN
    (SELECT IDCLIENT, DATA AS MOBILEMONEY FROM MOBILEMONEY) MOBILEMONEY;
...
CREATE OR REPLACE VIEW STATOFFREJOURNALIER AS
    ...
    WHERE OFFRE.IDOFFRE != 'MOBILEMONEY' AND OFFRE.IDOFFRE !='DEFAUT';
...
SELECT * FROM OFFRE,JOUR 
    NATURAL LEFT JOIN (SELECT IDOFFRE, EXTRACT(DOW FROM DATEACHAT) JOUR, COUNT(*) NOMBRE FROM ACHATOFFRE GROUP BY IDOFFRE, EXTRACT(DOW FROM DATEACHAT)) WHERE IDOFFRE != 'MOBILEMONEY' AND IDOFFRE !='DEFAUT';
